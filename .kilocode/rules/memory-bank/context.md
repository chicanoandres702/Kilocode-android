...
| 2026-06-28 | Added Stop button during generation (prominent red button calling `abortSession`), autonomous mode uses WorkManager (`PromptWorker`) for background prompt execution that survives app backgrounding/phone-off. Created `PromptForegroundService` for foreground notification. RepoScreen set as start destination. Added FOREGROUND_SERVICE_DATA_SYNC and POST_NOTIFICATIONS permissions. All session API calls pass directory as query param. Verified on emulator: build, install, app launch all successful. |
| 2026-06-28 | Implemented planning milestones/issues feature: `PlanningRepository` with in-memory cache (5-min TTL), `PlanningScreen` with cascading milestone→issue→description UI, `PlanningWizardScreen` with 4-step guided wizard, backend barrel route at `/api/planning` using `gh` CLI, navigation integration from HomeScreen. Created branch `feature/49-planning-milestones-issues`, committed and pushed. PR #50 opened. |
